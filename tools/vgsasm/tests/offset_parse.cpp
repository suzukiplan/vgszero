...
{
    try {
        test_normal("offset(hoge.foo)", 1, TokenType::Offset, "HOGE.FOO");
        test_error("offset hoge.foo)", "No `(` in offset syntax.");
        test_error("offset (+)", "No structure name specified in offset syntax.");
        test_error("offset (hoge)", "No structure field name specified in offset syntax.");
        test_error("offset (hoge.foo", "No `)` in offset syntax.");
    } catch (...) {
        return -1;
    }
